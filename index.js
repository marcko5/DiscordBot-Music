        ...
        if (int.commandName == "setup"){
            ...
            if (command){
                try{
                    ...
                    int.editReply({ embeds: [getBasicEmbed(client, "error", "(❌) Error", `While processing **${int.commandName}** an error occured..\nAnyway, **try again** later!`)], ephemeral: true });
                        ...
                        if (!("command" in rooms[msg.guild.id])){
                            ...
                            if (!checkCommand){
                                ...
                                if (msg.channel.id != rooms[msg.guild.id].command)
                                    ...
                                    if (num < 1 && num > cache[msg.guild.id][msg.author.id].result)
                                        msg.reply({ embeds: [getBasicEmbed(client, "error", "(❌) Error", `While processing **search request** an error occured..\nYou **typed wrong** answer!`)] });
                                    else{
                                        var voiceChannel;
